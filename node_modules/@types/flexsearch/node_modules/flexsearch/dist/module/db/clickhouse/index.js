...
ClickhouseDB.prototype.open = async function () {
    ...
    if (!this.db) {
        this.db = Index || (Index = new ClickHouse(defaults));
    }
...
ClickhouseDB.prototype.commit = async function (flexsearch, _replace, _append) {
    ...
    if (flexsearch.store) {
        ...
        if (data.length) {
            promises.push(this.db.insert(`INSERT INTO ${this.id}.reg (id, doc)`, data).toPromise());
        }
        ...
        if (data.length) {
            promises.push(this.db.insert(`INSERT INTO ${this.id}.reg (id)`, data).toPromise());
        }
    }
